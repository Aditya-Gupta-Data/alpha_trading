...
| 09:30 Saturday | `src.ingestion.scrip_master` | SCRIP RECONCILIATION: diffs every `SECURITY_ID_MAP` id against Dhan's public scrip master; de-duped review card on any mismatch. Replaces the old "re-verify IDs by hand on every deploy" rule. Log: `logs/scrip_master.log` |
