...
| `scripts/push_token_to_vm.sh` | Pushes ONLY the freshly-renewed `DHAN_ACCESS_TOKEN` to the VM (cron 07:10 IST, 10 min after local `src.renew_token`) — never `DHAN_PIN`/`DHAN_TOTP_SECRET`/`DHAN_API_KEY`/`DHAN_API_SECRET`, which stay Mac-only. Token crosses via a chmod-600 temp file over `gcloud compute scp` (never embedded in a command line), the VM updates its own `.env` via `renew_token.replace_token` (same safe single-line-replace + `.env.bak`), then restarts `alpha-trading.service`. `--dry-run` to preview. Requires `gcloud` authenticated as `adigupta1998@…` against the VM. |
