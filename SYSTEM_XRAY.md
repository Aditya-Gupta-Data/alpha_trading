...
| 4 | `data/fundamental_reports/` PDFs + `report_downloader` cron | 267+ ticker folders | its reader (`annual_report_analyzer`) is in `research_archive/` | **STOP THE CRAWL.** Keep the PDFs (re-fetching NSE is slow and rude), but a throttled NSE crawl feeding an archived module is pure cost. |
