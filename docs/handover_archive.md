# HANDOVER_ARCHIVE.md — the historical cold-start record

**This file is the continuation of `HANDOVER.md`, split out on 2026-08-11.**
Nothing was deleted, rewritten or summarised: every dated block below is
verbatim as it was written, and they remain **reverse-chronological** — the
newest archived block is first, and it directly follows the oldest block still
living in the active `HANDOVER.md`.

**Why the split.** `HANDOVER.md` is a blocking first read under `CLAUDE.md`
RULE 1, so its full 3,734 lines were being re-ingested at every session start
(~56k tokens) even though only the top block was current state. The active file
now carries the last two sessions plus an explicit **PENDING ISSUES / BACKLOG**
section; everything older lives here.

**Read this file when** you need the reasoning behind something the active
handover references but does not explain, or when you are checking whether a
problem is genuinely new. Search it before declaring anything a first
occurrence — most things here happened before.

> **The RULE 3 guarantee still holds.** This is append-only history. Do not
> edit these blocks; add new ones only by moving them down from
> `HANDOVER.md` as they age out.
