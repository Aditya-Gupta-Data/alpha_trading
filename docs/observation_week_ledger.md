...
- **Equity card showed `—` for LAST/P&L on all 3 positions (15:45 EOD, 2026-08-04) — NOT a broken fetcher; a fail-quiet seam plus a coverage gap. Four fixes shipped.** Owner reported the EOD card printing `(3 unmarked)` with dashes for FINEORG/SUPREMEIND/WABAG and diagnosed it as the live price fetcher. **The fetcher was fine:** a step-by-step probe on the VM (`/tmp/probe_fineorg.py`) passed every gate — `market_data_muzzled()` False, `security_id_for(FINEORG)='3744'`, `get_live_price_by_id(3744)=4929.5` — and `render_book_lines()` re-rendered fully populated 12 minutes later, with `unrealized` visibly moving across runs (-1,030 → -575 → +278 → -275). The failure was REAL but TRANSIENT at 15:45, a contention point where `chain_archiver` (15:40), `intraday_tracker` (15:45) and `eod_summary` (15:45) share one Dhan account. **Why it was undiagnosable: `equity_desk.live_quote` swallowed every exception** — a rate-limit rejection, an expired token and a socket timeout all reached the card as the same bare `—`. The premise that a "CAS auction system" change altered symbols or the API schema was NOT supported: all three tickers failed identically and simultaneously, which is systemic, never three unrelated per-security corporate actions; `security_id_for` reads a LOCAL id map, so a provider symbol change cannot blank it; and the options book reported "1 open, 1 marked live" throughout. **THE DEEPER GAP the owner then identified — "why not use the price we already fetched?": there wasn't one.** The desk's darlings are in NO capture job: `intraday_15m.jsonl` carries the 84-name OPTIONS watchlist (0 rows for all three desk names), `data/lake/candles/` holds only indices, and the one file that *looked* like a cache — `equity_desk_snapshot.json` — was 14 days stale with `last_close == entry_price` on every row and positions the desk no longer holds (MACPOWER, WAAREERTL; FINEORG qty 8 vs 3). Falling back to it would have printed confident WRONG numbers, which is worse than a dash. **SHIPPED:** (1) `live_quote` now NAMES its failures on stderr (`no_security_id` / provider-returned-no-price / `<ExcType>: msg`) while still failing open; (2) `capture()` sweeps the desk's open funded book every 15 minutes through the scrip-id door (`src="dhan_live_15m_desk"`), so exit logic has a durable series; (3) NEW `capture_darlings()` — one daily close for ALL ~105 darlings, VM cron #26 at 15:50 IST, so ENTRY zones are visible for names we do not yet hold; (4) `equity_desk_snapshot.json` PARKED (not deleted) on both machines. **Regression caught during the build, worth recording on its own:** the new `desk_tickers()` default read the REAL open book from inside pytest — six existing `capture()` tests instantly grew 5 phantom desk failures. Fixed with the standing pytest muzzle. That is the fourth instance of the same family (07-22 journal drift, 07-23 digest-queue leak, 07-27 opportunity-cost rows): **a new default that reaches live state is the single most repeated defect in this codebase.** 8 new tests; suite 1,700.

- **`equity_desk_snapshot.json` PARKED, provenance established — 2026-08-04.** Owner asked whether it was a test artifact (delete) or something real (park). It was **real**: the Mac→VM transport built under decision **#82** (2026-07-21, ONE FIRM VIEW) and superseded HOURS LATER the same day by **#83**, which made the desk VM-native — `render_book_lines`'s own docstring reads "All VM-local; no snapshot, no SSH (decision #83)". Its `publish_snapshot`/`load_snapshot`/`render_firm_lines` machinery is already gone from `src/`; only the orphaned data file survived on both machines, git-ignored and referenced by nothing outside historical MODULES/DECISIONS prose. Renamed to `.parked` on the Mac AND the VM; the equity card re-rendered fully populated immediately afterwards, confirming no live path reads it. **Delete after a clean observation period — not before.**

- **Two 07-30 predictions verified on 08-01 (both were explicitly left pending in this ledger).**
  - **Task K edge decay — CONFIRMED as backlog, not breakage.** The 07-30 entry predicted "`expired` should collapse to ~0 from tomorrow now that the clock is current". The 07-31 20:00 sweep: `{'swept': 55, 'decayed': 55, 'expired': 0}` — versus 07-30's `{'swept': 86, 'expired': 45}`. Zero expirations on the second pass, and the 11 resurrected `concentrates_in` edges survived on λ=0.002 as designed. The swept count fell 86→55 because 45 edges are now inactive and 11 came back. **Structural amnesia closed; the prediction held exactly.**
  - **`src.suggest` DH-905 retry — deployed and clean, but the retry path is NOT yet proven.** The 07-30 entry said "Verification pending: the next VM 08:00 run after deploy". First post-deploy run (Fri 07-31 08:03): **0 DH-905, 0 skips, 0 recoveries**, HDFCBANK.NS analysed normally (`downtrend, RSI 42, ₹754.0`) instead of being dropped, run ending in a plain `Done.` **State it precisely: the transient Dhan-side window simply did not fire that morning, so nothing failed and the retry pass had nothing to catch.** No names were lost — but the recovery path and the widened error print are both still untested in production and will only prove out the next time the window hits. Deployed ≠ proven.
  - **NEW open item — the Stage-B tracker does not model NSE holidays.** Slack against the decision-#86 Oct-13 target is pinned at **0** (52 weekdays available vs 52 sessions needed as of 08-01), and the tracker's own output labels the estimate "holidays not modelled". Ganesh Chaturthi, Gandhi Jayanti and the Diwali cluster all fall inside the window, so the 60th session realistically lands PAST Oct 13 on holidays alone, with zero operational failures. Aug 15 is a Saturday this year and costs nothing. Not acted on — either model the NSE holiday calendar in the tracker so the date reflects reality, or accept Oct 13 as approximate. Owner's call.

- **THE THIRD LAYER, and the real one: the STATISTIC itself was broken — `co_stress` redefinition, owner-ruled and shipped 2026-08-01 (same day, second session).** The three structural fixes below (ragged missingness, circular shift, O(n) null) were built, tested (bit-identical `_z_series` proof included) and WORKED — the full 64-year lake ran end-to-end for the first time — and the gate STILL admitted 0/25, with the null WORSE than before (circular median 8.98 vs the old block's 4.66, real max only 6.25). That ruled out the surrogates and exposed the layer beneath: **`system_stress` = RMS of |z| over the channels PRESENT is a variable-divisor statistic, incomparable across coverage eras.** One channel at |z|=15.6 on a 2-channel day scores RMS 11.0 and outranks COVID (4 channels, 5.27). Verified directly: every surrogate maximum landed on 2-channel days at ~11.0 while every real crisis sits on 3-4-channel days at 5-6 — the null was measuring "how thin can coverage be when a big single move lands", and real crises were penalised precisely because they occur when MORE channels report (denser RMS = diluted). Three knock-ons, all confirmed on real data: (1) six of AD-1's top-25 "shocks" were single-channel US10Y ghosts from 1962-72 — **AD-1's own ranking was contaminated by the same defect**; (2) crisis anchors were MIS-DATED toward thin-coverage days; (3) no surrogate null of any design could be specified for it.

  **THE FIX (owner ruling, validated by a 50-surrogate empirical preview BEFORE production code was written): `co_stress` = the SECOND-LARGEST |z| among channels present, with a ≥3-channel eligibility floor (`MIN_CHANNELS`).** Second-max is the direct measurement of simultaneity — large only when at least TWO assets are in shock at once; a lone spike scores ~0 because the second mover was quiet; no divisor, nothing to dilute. The floor makes the scannable window 1988-06+ (you cannot measure a system with one bond yield — the pre-1973 lake IS one bond yield). Preview results that justified shipping it: the null became beatable AND graded for the first time (circular median 3.74 vs real max 5.37; p spans 0.0588→1.0 monotone in the statistic, versus flat 1.0000 under RMS); the 1960s ghosts vanished; **crisis anchors moved to the historically correct days — COVID 2020-05-19→2020-03-18 (the actual panic peak), GFC 10-22→10-24, taper 05-29→05-28** — the strongest validation available, since nothing in the pipeline knows those dates; and **1991-07-03 (old #1) demoted honestly to co-stress 0.36** — in this lake July 1991 is a USDINR devaluation, ONE channel, and a truthful simultaneity statistic says so. New all-time top-3 under the valid instrument: COVID 2020-03-18 (5.37), 2011-09-22 US-downgrade/euro (3.78), **2026-03-27 (3.59) — the unlabelled anomaly, now #3 in 38 years**. `rank_shock_candidates`, `_max_stress_of`, `significance_gate` and `gate_many` all moved to co-stress with the SAME floor on both observed and null sides (a null computed under different conditions than the observation is not a null — this session's lesson, twice). `system_stress` kept as a reference implementation, marked superseded, called by nothing. 5 new tests incl. the ghost-killer (a violent single-channel spike must score <3, not 11) and a mixed-era lake asserting no single-channel-era anchor leaks into the ranking. Suite 1,691. **Definitive 500-surrogate run, same session — FINAL VERDICT: 0 ADMITTED, and the number is honest.** COVID 2020-03-18 (co-stress 5.37) SPLIT the two nulls: it clears circular-shift at **p = 0.0419** (20 of 500 surrogates beat it) and fails phase-randomized at **p = 0.0579** (28 of 500) — eight thousandths over the line. The AND-gate refuses, which is "one null is not enough" doing precisely its job: the phase surrogate preserves each channel's full autocorrelation spectrum, and heavy-tailed series under preserved autocorrelation produce slightly larger chance alignments than rotation (its null max 15.20 shows the tail). The defensible statement on the record: over 38 years of 4-channel history, even COVID's cross-asset co-movement sits at ~the 95th percentile of chance alignment — strong, not conclusive at the bar we set BEFORE seeing the result. **2026-03-27 resolved as a watch item, not a regime: p 0.36-0.58 under both nulls** — #3 co-movement in 38 years yet comfortably inside chance; if it is the start of something, forward data will say so. Nothing written to `discovered_episodes.json` (nothing admitted; `route_to_court` correctly has nothing to route). Verdict shipped as-is per the kill-criterion doctrine — the system is designed to disappoint us honestly, and today it did.

- **AD-2's significance gate is STRUCTURALLY UNABLE TO ADMIT ANYTHING — first real-lake run, 2026-08-01. Two independent defects, neither reachable by the unit tests.** Owner ruled "run it in the wild before building more" (Option 1 over building the motif gate / registry enrolment). AD-1 passed with distinction; AD-2 failed twice. **[SAME-DAY UPDATE: both defects FIXED in the second session — and fixing them exposed a third, deeper one; see the co_stress entry above. The block-bootstrap "splice" diagnosis below was CORRECT but INSUFFICIENT: the circular shift eliminated every splice and the null was still unbeatable, because the statistic itself was.]**

  **(a) Ragged-missingness crash — AD-2 had NEVER been runnable on the real lake.** `significance_gate` dies with `IndexError` on every candidate. The real macro lake is ragged (over 16,846 aligned sessions: US10Y 100% populated → 1962, USDINR 82% → 1973, BRENT 60% → 1987, DXY 31% → 2006), and `block_bootstrap`/`phase_randomize` both DROP the None holes (`[v for v in series if v is not None]`), so each channel's surrogate comes back a DIFFERENT length (10223/5364/13974/16846). `_max_stress_of` then builds its index from whichever channel is first in the dict and indexes all four at `i` — the short one overruns. The 11 unit tests never caught it because synthetic test lakes are equal-length and fully populated. **A bounds check is NOT the fix:** `system_stress` measures cross-asset SIMULTANEITY (RMS across channels moving at once), so unequal-length surrogates are no longer time-aligned and the RMS denominator differs between the observed statistic and its null — the p-value would be computed under different conditions than the observation. The correct fix preserves the missingness pattern so the null matrix is exactly as ragged as the observed one. NOT applied this session (owner: get a real run first).

  **(b) The block-bootstrap null is MISSPECIFIED — the "Frankenstein splice" artifact.** Worked around (a) by restricting to the post-2006 full-observation window (2006-01-02→2026-07-28, 5,363 sessions, only 4 partial days dropped — 2008 GFC stays in scope), then ran 200 surrogates of each type. **Result: 0 admitted of 25.** The null explains why: **block-bootstrap max-stress median 4.66** (p95 6.24) versus COVID-2020, the most violent cross-asset event in the window, at **5.27** — the TYPICAL random surrogate nearly beats the worst real crisis. From candidate #6 down, `p_block` is a flat **1.0000**: all 200 surrogates beat the observed event. Mechanism: `block_bootstrap` concatenates random contiguous 20-day blocks, so every junction is an artificial discontinuity splicing one price regime into an unrelated one — and `system_stress` is built to detect precisely that (a large 20-day z-move across all channels at once). A 5,363-point surrogate carries ~268 such splices and the statistic takes the MAX over all of them, so the null measures "worst splice artifact in 268 tries" against "worst real event in 20 years". **The diagnostic that distinguishes misspecification from mere strictness: a too-strict-but-valid gate produces p-values clustered near the 1/201 resolution floor; these SATURATE at the ceiling.** The phase-randomized null is better behaved (median 3.12, and it does separate COVID at 0.2537 from the tail at 1.0000) because it preserves the power spectrum and creates no splices — but nothing beat it either.

  **What AD-1 proved, and it is the good news:** unsupervised over four cross-asset channels with ZERO human labels, it independently surfaced 2013-05-29 **taper tantrum** — the medoid of the human catalog's own A2 archetype — plus the GFC as a coherent multi-phase cluster (2008-01-23 / 2008-05-15 / 2008-10-22 / 2009-01-26), COVID 2020-05-19, Ukraine 2022-03-08, demonetisation 2016-11-23, the 2007-08-16 quant quake, and (full-lake run, pre-restriction) the **1991-07-03 India BoP crisis** at the highest stress in 64 years. The core physics are sound; the measuring instrument is not. **Held-out recurrence passed 24/25** (only COVID failed, correctly — train-head event with no comparable tail) so that component works.

  **UNRESOLVED and the most interesting thread: 2026-03-27, stress 3.65 — the second-highest in twenty years with no obvious human label.** `p_block 0.9204 / p_phase 0.3881` says nothing about whether it is real; the instrument cannot measure it. Re-test against a corrected null before drawing ANY conclusion.

  **Methodological caveat owned:** the null was built ONCE and shared across all 25 candidates (the null does not depend on the candidate — only `observed` does), because the per-candidate version needs ~23 min at even 10 surrogates (`_z_series` is O(n²)). Standard common-random-numbers; it changes no admission decision here, but the 25 p-values are correlated through one draw, so fine-grained rank differences between adjacent candidates are not meaningful.

  **OWNER RULING for the next session (not built): fix the statistical foundation properly** — (1) replace the block bootstrap with a **circular/stationary bootstrap** (one wrap-point instead of ~268 splices), (2) implement the **ragged-missingness fix** so the full 25-year history returns to scope. The two interact: with missingness fixed, the 1991 BoP crisis re-enters and is a far stronger test of any corrected null than anything post-2006. Nothing was written this session — no `discovered_episodes.json`, no repo data files; the only committed change is the corrected module docstring (it had claimed AD-2/3/4 were "SCAFFOLDED" long after `d495de3` built them — stale since 2026-07-23).
