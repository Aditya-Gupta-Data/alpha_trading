...
# "daily_context 25/40 frames" — which reads IDENTICALLY whether the corpus
...
def test_a_growing_corpus_reports_an_observed_rate_and_a_projected_date():
    ...
    assert g["days_to_go"] == 15                      # 40 - 25, at 1.0/day
    assert g["projected_ready"] == "2026-08-19"

    line = nightly.depth_reason(g)
    assert "25/40" in line and "15 more nights" in line and "2026-08-19" in line
...
def test_a_slower_corpus_projects_a_later_date_not_the_same_one():
    ...
    assert g["days_to_go"] == 30                      # (40-25)/0.5
...
def test_an_empty_table_never_divides_by_zero():
    ...
    assert nightly.depth_reason(g) == "daily_context 0/40 frames"
